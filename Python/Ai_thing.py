...
def analyze():
    ...
    image_path = "Saved_Images/img.jpg"
    response = chat_with_gpt(API_KEY, image_path, user_prompt)
    if response != "False":
        response = response.split(' ')
        print(response)
        try:
            n = len(os.listdir(f"Saved_Images/{response[0].lower()}"))
        except:
            os.makedirs(f"Saved_Images/{response[0].lower()}")
            n=0
        cv2.imwrite(f'Saved_Images/{response[0].lower()}/{response[0].lower()}{n}.jpg', cv2.imread("Saved_Images/img.jpg"))
        return True

    else:
        os.remove("Saved_Images/img.jpg")
        return False